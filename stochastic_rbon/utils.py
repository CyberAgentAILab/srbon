...
def load_dataset(dataset, ref=False, raw_text=False):
    ...
    if dataset == "alpaca":
        ...
        if not ref:
            ...
            if dataset == "hh-harmless":
                dataset = datasets.load_dataset("Anthropic/hh-rlhf", data_dir="harmless-base")['test']
            elif dataset == 'hh-helpful':
                dataset = datasets.load_dataset("Anthropic/hh-rlhf", data_dir="helpful-base")['test']
            elif dataset == "hh-harmless-t":
                dataset = datasets.load_dataset("Anthropic/hh-rlhf", data_dir="harmless-base")['train']
            elif dataset == "hh-helpful-t":
                dataset = datasets.load_dataset("Anthropic/hh-rlhf", data_dir="helpful-base")['train']
            else:
                assert False
            if not ref:
                lines = []
                for d in dataset:
                    instruction = d['chosen'].split("\n\nAssistant:")[0].split("\n\nHuman: ")[1]
                    message = [
                        {
                            "role": "user",
                            "content": instruction,
